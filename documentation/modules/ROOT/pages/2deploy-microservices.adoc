...
----
This annotation, combined with the fact that the `tutorial` namespace is part of the mesh (thanks to the *ServiceMeshMemberRoll* created in the Setup section) means that each microservice has an associated Istio "sidecar" to manage traffic as illustrated below. Read the Istio documentation to learn more about the https://istio.io/latest/docs/reference/config/networking/sidecar/[Sidecar].
...
If you installed the OpenShift Web Terminal, refresh the OpenShift Web Console and start a Web Terminal using the terminal icon in the top-right.
...
. Download the OpenShift CLI (`oc`) from the help menu using (*?* icon) in the top-right of the OpenShift Web Console.
+
...
First, create the Deployment for the customer application. Istio will inject a sidecar container when the customer service Pod is created, since the Pod spec in the *Deployment.yml* has the `sidecar.istio.io/inject: "true"` annotation applied.

. Deploy the customer application using the *Deployment.yml*:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
----
. Create a Service so the customer application has a stable DNS entry:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
The `spec.replicas` for in the *Deployment.yml* is set to 1, so why is there a second container in the deployed Pod? The reason is that Istio injected the sidecar container that will be used to manage network traffic!

You can use `kubectl describe pods -n tutorial` to inspect the customer application Pod and see the Istio annotations and that a second container was automatically added to the Pod spec by the Istio operator.
...
NOTE: This section is optional. Feel free to move on to the xref:3kiali.adoc[Observability section] of this guide.
