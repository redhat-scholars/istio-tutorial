...
To check it run `kubectl get virtualservice` and `kubectl get destinationrule` and if it is necessary run  `kubectl delete virtualservice virtualservicename -n tutorial{namespace-suffix}` and `kubectl delete destinationrule destinationrulename -n tutorial{namespace-suffix}`
