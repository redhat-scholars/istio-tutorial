...
NOTE: You may want to push some load onto the deployed applications in order to see some metrics.  Run `siege $CUSTOMER_URL` to produce load.  Setting CUSTOMER_URL with `CUSTOMER_URL=$(oc get route/customer -o jsonpath="{.spec.host}")`.

[source,bash,subs="+macros,+attributes"]
----
ifdef::workshop[]
open http://grafana-istio-system.{appdomain}/d/UbsSZTDik/istio-workload-dashboard?refresh=5s&orgId=1
or
firefox http://grafana-istio-system.{appdomain}/d/UbsSZTDik/istio-workload-dashboard?refresh=5s&orgId=1
endif::workshop[]
...
Make sure to select your $WORKSHOP_USER-istiotutorial namespace and customer, preference-v1 or recommendation-v1
...
[source,bash,subs="+macros,+attributes"]
----
container_memory_rss{namespace="$WORKSHOP_USER-istiotutorial",container_name =~ "customer|preference|recommendation"}
----
...
// Application Metrics removed from Workshop because it breaks kiali
// https://issues.jboss.org/browse/KIALI-2811
ifndef::workshop[] 
[#applicationmetrics]
=== Application metrics
