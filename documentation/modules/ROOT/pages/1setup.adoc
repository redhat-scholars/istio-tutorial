...
`curl -sL https://git.io/_has | HAS_ALLOW_UNSAFE=y bash -s minishift docker oc kubectl git mvn curl stern siege`
...
You have two options to install Istio in your environment. The option 1 is using the Istio source code from upstream community. The option 2 is through the installation of https://docs.openshift.com/container-platform/3.10/servicemesh-install/servicemesh-install.html[Red Hat OpenShift Service Mesh] 

[#upstreamistioinstallation]
== (Option 1) Upstream Istio installation
