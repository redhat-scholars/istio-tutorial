...
We can access Kiali at `kiali-istio-system.{appdomain}/console/`, so
let's do it:
...
The default credentials are "admin/admin", but for the workshop the proper credentials will be provided by your instructor.

[#generatingdata]
== Generating Sample Data

To show the capabilities of Kiali, you'll need an Istio-enabled application to
be running. For this, we can use the `customer` application we created
earlier.

To generate data for it, we can `curl` customer repeatedly with this command:
