...
or
kubectl create namespace istioegress
kubectl config set-context $(kubectl config current-context) --namespace=istioegress
...
or
kubectl apply -f <(istioctl kube-inject -f src/main/kubernetes/Deployment.yml) -n istioegress
...
[source,bash]
----
istioctl delete serviceentry httpbin-egress-rule google-egress-rule github-egress-rule -n istioegress
----
